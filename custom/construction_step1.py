# Brik Assemble @20250515

import rclpy
import DR_init
import time
import sys


# Configuration for a single robot
ROBOT_ID = "dsr01"
ROBOT_MODEL = "m0609"
VELOCITY, ACC = 60, 60
ON, OFF = 1, 0

# Initialize DR_init with robot parameters
DR_init.__dsr__id = ROBOT_ID
DR_init.__dsr__model = ROBOT_MODEL

rclpy.init()
node = rclpy.create_node("brick_assemble_node", namespace=ROBOT_ID)
DR_init.__dsr__node = node

try:
        from DSR_ROBOT2 import (
            get_digital_input,
            set_digital_output
        )

except ImportError as e:
        print(f"Error importing DSR_ROBOT2 : {e}")
        sys,exit()

############ Grippper Grip & Rease ##############
def grip():

    set_digital_output(1, 1)
    set_digital_output(2, 0)
    # 그리퍼 닫힘
   
       
def release():

    set_digital_output(1, 0)
    set_digital_output(2, 1)
    # 그리퍼 열림


############ Main ##############
def main(args=None):


    try:
        from DSR_ROBOT2 import (
            set_tool,
            set_tcp,
            movej,
            movel,
            trans,
            mwait,
            task_compliance_ctrl,
            set_desired_force,
            check_force_condition,
            release_force,
            release_compliance_ctrl,
            get_tool_force,
            DR_FC_MOD_REL,
            DR_AXIS_Z,
            DR_BASE,
        )

        from DR_common2 import posx, posj

    except ImportError as e:
        print(f"Error importing DSR_ROBOT2 : {e}")
        return


    ##### POSE DEFINITION ####
    # position for lego should be written 100 upward (Z + 100 from original position)
    # location : home 
    pos_home = posj(0, 0, 90, 0, 90, 0)


    # location : lego
    block_long = posx(379.32, 192.52, 233.65+100, 98.77, 179.74, 65.97)
    block_short = posx(295.97, 17.57, 269.64+100, 179.28, 180.0, 91.38)

    # location : row 1, column 1 (not setted yet)
    step_1 = posx(405.56, 128.88, 259.14, 23.12, -180, 24.17)
    # location : row 1, column 2 (not setted yet)
    step_2 = posx(516.63, 128.32, 258.67, 8.61, -180, 10.17)
    # location : row 1, column 3 (not setted yet)
    step_3 = posx(627.96, 126.02, 259.87, 43.33, 180, 44.96)

    # location : row 2, column 1
    step_4 = posx(403.77, 1.11, 259.86, 146.56, -179.97, 148.58)
    # location : row 2, column 2
    step_5 = posx(515.01, -0.64, 260.7, 176.19, 180, 178.2)
    # location : row 2, column 3 (not setted yet)
    step_6 = posx(625.36, 0.31, 262.03, 174.87, -179.96, 176.86)

    # location : row 3, column 1 (not setted yet)
    step_7 = posx(400.35, -125.82, 261.81, 2.8, -180, 4.77)
    # location : row 3, column 2 (not setted yet)
    step_8 = posx(512.38, -126.53, 259.75, 165.26, 179.99, 167.18)
    # location : row 3, column 3 (not setted yet)
    step_9 = posx(623.47, -128.45, 261.84, 167.59, 179.97, 169.52)

    # Just to go up
    block_to_down = posx(0,0,-100,0,0,0)

    # set_tool("Tool Weight_2FG")
    # set_tcp("2FG_TCP")

    movej(pos_home, vel = 30, acc = 30)
    release()
    estate = [1,2,3,1,2,3,1,2,3]
    construction_step = 1

    while rclpy.ok():
        for buildings in estate:

            # REQUESTED BUILDING CONSTRUCTIONS CHECK 
            if buildings == 0:
                print(f"{construction_step}단계에 요청받은 건설의뢰가 존재하지 않아 다음 단계로 넘어갑니다.")
                construction_step += 1
                continue
            elif buildings == 1:
                block_for_construction = block_short
            elif buildings == 2:
                block_for_construction = block_long

            # POSITION CHECK FOR BUILDING CONSTRUCTION
            if construction_step == 1 :
                block_to_place = step_1
            elif construction_step == 2 :
                block_to_place = step_2
            elif construction_step == 3 :
                block_to_place = step_3
            elif construction_step == 4 :
                block_to_place = step_4
            elif construction_step == 5 :
                block_to_place = step_5
            elif construction_step == 6 :
                block_to_place = step_6
            elif construction_step == 7 :
                block_to_place = step_7
            elif construction_step == 8 :
                block_to_place = step_8
            elif construction_step == 9 :
                block_to_place = step_9

            # REPEAT UNTIL 4TH FLOOR CONSTRUCTED.
            for floor in range(4):

                # GET BLOCKS FOR CONSTRUCTION
                movel(block_for_construction, vel = 30, acc = 30)
                movel(block_to_down, vel = 30, acc = 30, mod=1) # mod=1 : 상대좌표
                mwait(0.5)
                grip()
                mwait(0.5)
                movel(block_for_construction, vel = 30, acc = 30)

                # CONSTRUCT
                movel(block_to_place, vel = 30, acc = 30)

                ret = task_compliance_ctrl(stx=[500, 500, 500, 100, 100, 100])

                time.sleep(1)
                ret = set_desired_force(fd=[0, 0, -20, 0, 0, 0], dir=[0, 0, 1, 0, 0, 0], mod=DR_FC_MOD_REL)

                time.sleep(1)
                force_condition = check_force_condition(DR_AXIS_Z, max=30)

                while (force_condition > -1): # 힘제어로 블럭 놓기
                    force_condition = check_force_condition(DR_AXIS_Z, max=10)

                release()
                mwait(0.5)
                movel(block_to_place, vel = 30, acc = 30)

            print(f"{construction_step}단계 완료, 다음 단계로 넘어갑니다.")
            construction_step += 1

    rclpy.shutdown()
if __name__ == "__main__":
    main()
