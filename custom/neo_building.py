...
with open('/home/rokey/ros_ws/building_text/blue_print_index.txt', 'r', encoding='utf-8') as file:
    i = int(file.read())
    print(f'측정중인 공간: {i}')

with open('/home/rokey/ros_ws/building_text/construct_index.txt', 'r', encoding='utf-8') as file:
    construct_index_str = file.read()
    construct_index = ast.literal_eval(construct_index_str)
    print(f'현재 건축중인 건물: {construct_index}')
...
cup_down = posx(456.61, 267.75, 165 - 100, 16.53, -179.87, 16.16)
xlist = [# Pull cement
            # 0
            posx(457.41, 268.36, 175.0, 104.04, 179.98, 103.65),
            posx(456.61, 267.81, 175.0, 16.53, -179.87, 16.16),
            # 1
            posx(395.87, 144.91, 175.0, 90.38, -127.98, 90),
            posx(395.87, -183.79, 175.0, 90.38, -127.98, 90),
            #1-2
            posx(415.86, -224.88, 175.0, 90.38, -127.98, 90),
            posx(448.35, -224.89, 175.0, 90.38, -127.98, 90),
            #2
            posx(474.69, -178.35, 175.0, 90.38, -127.98, 90),
            posx(474.68, 136.46, 175.0, 90.38, -127.98, 90),
            #2-3
            posx(516.84, 162.17, 175.0, 90.39, -127.98, 90),
            #3
            posx(557.77, 124.13, 175.0, 90.39, -127.98, 90),
            posx(557.78, -193.65, 175.0, 90.39, -127.98, 90),
            ]
...
def cement():
    ...
    # step 2 mixing
    print('시멘트 몰탈 시작')
    movej(pos_home, vel=VELOCITY, acc=ACC) # 섞는 위치 이동하기
